...
JSON_PATH = os.path.join(BASE, "oregon_critters_filtered_20k.json")
...
def download_one(im):
    # source_path is the original nested S3 key; file_name is the flat local name
    # (see select_and_download.py).
    key = PREFIX + im["source_path"]
    dest = os.path.join(IMAGES_DIR, im["file_name"])
